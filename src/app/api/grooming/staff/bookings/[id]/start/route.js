    ...
    try {
        ...
        // ---------------------------------------------------------
        // 7. Atomically start booking
        // ---------------------------------------------------------
        const { data: transition, error: transitionError } =
        await supabaseAdmin.rpc('start_grooming_booking', {
        ...
        if (transitionError) {
        console.error(
            'Start grooming booking RPC error:',
            transitionError
        )
        ...
        {
            success: true,
            message: 'Grooming service started',
        ...
        console.error('Grooming start-service error:', error)

        return NextResponse.json(
        {
            error: 'Failed to start grooming service',
