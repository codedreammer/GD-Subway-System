...
export default function VendorItemsPage() {
  ...
  return (
    <div className="min-h-screen bg-slate-50 pb-56">
      ...
      {vendor && vendor.is_online === false && (
        <div className="mx-5 mt-4 rounded-2xl bg-red-50 p-4 text-center font-bold text-red-600 shadow-sm border border-red-100">
      ...
      <div className="space-y-4 px-5 pt-5">
        {!items.length ? (
          ...
          items.map((item, index) => {
            ...
            return (
              <article key={item.id} className={`premium-card overflow-hidden p-4 ${vendor?.is_online === false ? "opacity-60 grayscale transition-all" : ""}`}>
                <div className="flex gap-4">
                  ...
                  <div className="flex min-w-[108px] flex-col items-end justify-between gap-3">
                    ...
                    {qty === 0 ? (
                      <button
                        ...
                        disabled={vendor?.is_online === false}
                        className="premium-button flex items-center gap-2 px-5 py-2.5 text-sm font-semibold disabled:cursor-not-allowed disabled:opacity-50"
                      >
                        <Plus size={16} />
                        Add
                      </button>
                      ...
                      <div className="flex items-center gap-3 rounded-2xl border border-green-200 bg-green-50 px-2 py-2 shadow-sm">
                        <button
                          ...
                          disabled={vendor?.is_online === false}
                          className="flex h-8 w-8 items-center justify-center rounded-xl bg-white text-green-800 transition hover:scale-105 hover:bg-green-100 disabled:cursor-not-allowed disabled:opacity-50"
                        >
                          <Minus size={16} />
                        </button>
